...
### `value`
It is what the user will type. It is not a big number, just plain humanized number, not scaled with the decimals. 1 USDC = 1000000 wei. This should be a `useState` value.
```tsx {11}
import {Input}  from '@yearn-finance/web-lib/components';
import {format} from '@yearn-finance/web-lib/utils';
...
### `onSetValue`
Trigger a change in the value. It will be a string, not a big number. This should be a useState setter. This is triggered everytime the user type something or when he clicks MAX.
```tsx {12}
import {Input}  from '@yearn-finance/web-lib/components';
import {format} from '@yearn-finance/web-lib/utils';
