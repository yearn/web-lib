 ...
 */
export type TWalletConnectOptions = Omit<Parameters<typeof WalletConnectProvider.init>[0], 'rpcMap'> & {
	/**
 ...
 */
export type TWalletConnectConstructorArgs = {
	actions: Actions
	/** Options to pass to `@walletconnect/ethereum-provider`. */
	options: TWalletConnectOptions
...
export class WalletConnect extends Connector {
	...
	declare public provider: WalletConnectProvider | undefined | any;
	public readonly events = new EventEmitter3();

	private readonly options: Omit<TWalletConnectOptions, 'rpcMap' | 'chains'>;

	private readonly rpcMap?: { [key: number]: string | string[] };
	...
	constructor({actions, options, defaultChainId, timeout = DEFAULT_TIMEOUT, onError}: TWalletConnectConstructorArgs) {
	...
	private disconnectListener = (error: any): void => {
