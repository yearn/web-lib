...
export async function getBestUrlMap(
	rpcMap: { [key: string]: string | string[] },
	timeout: number
	...
	return new Promise((resolve) => {
		...
		urls.forEach((url, i) => {
			...
			void promise
				...
				.catch(() => false)
				.then((success) => {
					// if we already isResolved, return
					if (isResolved) {
						return;
					}
					...
					// if this is the last call and we haven't isResolved yet - do so
					if (Object.keys(successes).length === urls.length) {
						const index = Object.keys(successes).findIndex((j) => successes[Number(j)]);
						// no need to set isResolved to true, as this is the last promise
					...
					new Array<number>(prospectiveIndex).fill(0).every((_, j) => !successes[j])
						) {
							isResolved = true;
 ...
 */
export function getChainsWithDefault(chains: number[], defaultChainId: number): [number, ...number[]] {
