...
This library is made for React projects with the idea to be light, efficient and easy to use for any Web3 projects.  
In this library you will find some basic components, some contexts, hooks and some other useful utils. We are working on this library to be the easy start for Web3, all done with the Blockchain ecosystem in mind.
...
[Get Started](/docs/1-install/intro) · [Examples](/examples/connect-wallet) ·
...
In this example, we wrap our whole nextJS app inside the `WithYearn` context, which enable all the feature we may need to work with it! A few configs are available, and you can check them [here](/docs/contexts/withYearn), but in short with this abstract wrapper, you gain access to the whole Web3 capabilities, some UI and theme controls, some global settings to access and update the Yearn's default ecosystem contracts and API, but also some more specific global contexts such as the Balances and the Price ones.  
  
Next, anywhere in our app, we can use the `useWeb3` hook to access to the Wallet, to the provider (aka link to the blockchain) and much more! If your user's `isActive`, it's wallet and provider will be ready to use in your dApp. Otherwise, you can just trigger the `openLoginModal` function to open a ready-to-use modal for the user's to connect to their wallet.  
...
- [Yearn Buyback](https://buyback.yearn.finance/), a project to buy your YFI.
...
- [Yearn Hack](https://hack.yearn.farm/), your UI to deploy all your needs to hack Yearn.
...
- [Keep3r Network](https://keep3r.network/), a decentralized infrastructures for web3 jobs and tasks
